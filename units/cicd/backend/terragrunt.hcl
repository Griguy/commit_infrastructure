...
include "inputs" {
  path = find_in_parent_folders("unit_configs/cicd/backend/config.hcl")
}
...
inputs = {
  ecr_repository_url       = dependency.ecr.outputs.repositories["cm-backend"].repository_url
  ecr_repository_arn       = dependency.ecr.outputs.repositories["cm-backend"].arn
